...
import { ArrowLeft, Wallet, Banknote, CreditCard, ChevronRight, CheckCircle2, ShieldCheck, Check, AlertCircle } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  // ==========================================
  // PORTED JAVA LOGIC: Validator & Styler
  // ==========================================
  const getCardInfo = (number: string) => {
    const cleanNum = number.replace(/\D/g, ''); // Removes spaces
    
    // 1. Porting: getCategory() & Dynamic Styling
    let type = "CARD";
    let gradient = "from-gray-700 to-gray-900 dark:from-gray-800 dark:to-black text-white";

    if (cleanNum.startsWith("4")) {
      type = "VISA";
      gradient = "from-blue-700 to-blue-900 text-white shadow-blue-900/40";
    } else if (cleanNum.startsWith("5")) {
      type = "MasterCard";
      gradient = "from-orange-500 to-red-600 text-white shadow-red-900/40";
    } else if (cleanNum.startsWith("34") || cleanNum.startsWith("37")) {
      type = "American Express";
      gradient = "from-sky-100 to-gray-300 text-slate-900 shadow-gray-500/40"; // Light theme for AMEX
    } else if (cleanNum.startsWith("6")) {
      type = "Discover";
      gradient = "from-orange-400 to-orange-600 text-white shadow-orange-900/40";
    }

    // 2. Porting: Luhn Algorithm (validityChecker, first/secondDigitRightToLeft)
    let sum = 0;
    let isSecond = false;
    for (let i = cleanNum.length - 1; i >= 0; i--) {
      ...
      if (isSecond) {
        d = d * 2;
        // Port of: if(number > 9) number = number/10 + number%10;
    ...
    // Port of: validate() length check & addTotal1Total2 modulus check
    const isValid = cleanNum.length >= 13 && cleanNum.length <= 16 && sum % 10 === 0;

    return { type, gradient, isValid, length: cleanNum.length };
  };
  ...
  // ==========================================
  // VIEW 2: ADD NEW CARD (Dynamic Background & Validation)
  // ==========================================
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-zinc-950 pb-32 animate-in slide-in-from-right-4 duration-300">
      ...
      <div className="px-4 md:px-8 max-w-3xl mx-auto pt-4 space-y-8">
        ...
        {/* DYNAMIC CARD MOCKUP */}
        {/* We inject cardInfo.gradient into the wrapper's className to dynamically color it */}
        <div className={`w-full aspect-[1.6/1] bg-gradient-to-br ${cardInfo.gradient} rounded-[24px] p-5 sm:p-6 shadow-2xl flex flex-col justify-between relative overflow-hidden transition-all duration-500`}>
          ...
          <div className="flex justify-between items-start relative z-10">
            ...
            <span className="font-extrabold italic text-lg sm:text-xl tracking-wider">{cardInfo.type}</span>
          </div>
          ...
          <div className="relative z-10 space-y-3 sm:space-y-4 w-full">
            ...
            <div className="flex justify-between items-end gap-3 w-full">
              <div className="min-w-0 flex-1 overflow-hidden">
                <p className="text-[8px] sm:text-[10px] opacity-70 uppercase tracking-widest sm:tracking-wider mb-0.5 sm:mb-1 whitespace-nowrap truncate">
                  Card Holder Name
                </p>
              ...
              <div className="shrink-0 text-right pl-2">
                <p className="text-[8px] sm:text-[10px] opacity-70 uppercase tracking-widest sm:tracking-wider mb-0.5 sm:mb-1 whitespace-nowrap">
                  Expiry Date
                </p>
        ...
        <div className="space-y-4">
          ...
          <div>
            {/* Validation Indicator injected into the label using ported Java check */}
            <div className="flex justify-between items-end mb-1.5">
              <label className="text-[12px] font-bold text-gray-700 dark:text-gray-300 ml-2 block">Card Number</label>
              {cardInfo.length > 12 && (
                <span className={`text-[11px] font-bold flex items-center gap-1 ${cardInfo.isValid ? "text-green-500" : "text-red-500"}`}>
                  {cardInfo.isValid ? <><Check className="w-3.5 h-3.5"/> Valid</> : <><AlertCircle className="w-3.5 h-3.5"/> Invalid</>}
                </span>
              )}
            </div>
            <input 
              ...
              value={cardNumber}
              onChange={(e) => {
                // Keep spaces format visually while typing
                const val = e.target.value.replace(/\s+/g, '').replace(/[^0-9]/gi, '');
                const formatted = val.match(/.{1,4}/g)?.join(' ') || val;
                setCardNumber(formatted);
              }}
              maxLength={19}
              className={`w-full bg-white dark:bg-zinc-900 border rounded-[16px] px-4 py-3.5 text-[15px] font-medium text-gray-900 dark:text-white outline-none transition-colors font-mono ${
                cardInfo.length > 12 && !cardInfo.isValid ? 'border-red-500 focus:border-red-600' : 'border-gray-100 dark:border-zinc-800 focus:border-gray-300 dark:focus:border-gray-600'
              }`} 
            />
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[100] bg-white dark:bg-zinc-950 p-4 pb-safe-offset-4 border-t border-gray-100 dark:border-zinc-800">
        <div className="max-w-3xl mx-auto">
          <button 
            onClick={() => {
              if (!cardInfo.isValid) return; // Prevent submission if invalid
              setPaymentMethod("card");
              setView("summary");
            }} 
            disabled={!cardInfo.isValid && cardInfo.length > 0}
            className={`w-full py-4.5 rounded-[18px] font-bold text-[16px] transition-all ${
              cardInfo.isValid 
                ? "bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:opacity-90 active:scale-[0.98]" 
                : "bg-gray-200 dark:bg-zinc-800 text-gray-400 cursor-not-allowed"
            }`}
            style={{ padding: '1.125rem' }}
          >
            {cardInfo.isValid ? "Add Valid Card" : "Enter Valid Card Details"}
          </button>
        </div>
      </div>
    </div>
