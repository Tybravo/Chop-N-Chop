...
import AIChatModal from "@/components/customer/AiChatModal"; // Adjust relative path to match your layout

interface HomeSearchPromptProps {
  suggestions?: string[];
}

export default function HomeSearchPrompt({ 
  suggestions = ["Jollof", "Pizza", "Smoothies"] 
}: HomeSearchPromptProps) {
  ...
  const handleSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    const conversationalKeywords = [
      "recommend", "best", "what should", "hungry", "craving", 
      "suggest", "help", "something", "spicy", "cheap", "lunch", "dinner", "breakfast"
    ];

    const isConversational = conversationalKeywords.some(keyword => queryLower.includes(keyword)) || queryLower.includes("?");

    if (isConversational) {
      setAiInitialQuery(query);
      setIsAiModalOpen(true);
      setQuery(""); // Clears search bar immediately when AI modal triggers
    } else {
      ...
      <section className="w-full" aria-labelledby="home-search-title">
        ...
        <form onSubmit={handleSearch} className="flex items-center gap-2 rounded-[18px] border border-gray-100 bg-white px-4 py-2 shadow-sm focus-within:border-[#FC6B31] dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <input
            ...
            placeholder="Search meals or ask AI (e.g. Best lunch for today?)..."
